...
typename vector<value_type>::reference vector<value_type>::at(size_type pos) {
  if (pos >= buf_) throw std::out_of_range("Out of range");
  return vector_[pos];
}

template <class value_type>
typename vector<value_type>::const_reference vector<value_type>::front() {
  if (empty()) throw "Is empty";
  return vector_[0];
}
...
typename vector<value_type>::size_type vector<value_type>::max_size() {
  return std::numeric_limits<size_type>::max() / sizeof(value_type) / 2;  //max()-bit sizeof-byte -> / 2
}
...
void vector<value_type>::erase(iterator pos) {
  ...
  for (iterator i = pos; i < end(); ++i) {   //{1,2,3,4,5} => {1,2,3->4,4->5} 
    *i = *(i + 1);
  }
  size_--;
}
